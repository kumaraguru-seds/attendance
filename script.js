...
async function renderMemberTimetable() {
    ...
    try {
        ...
        data.meetings.sort((a,b)=>{
            const d1 = new Date(`${a.date} ${a.startTime || "00:00"}`);
            const d2 = new Date(`${b.date} ${b.startTime || "00:00"}`);
            return d1 - d2;
        });
        ...
        Week's Meetings ( Sun - Sat)
        </h3>

        <div class="grid-container"
        style="grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:15px;">
        `;
        ...
        data.meetings.forEach(m => {
            ...
            const startTime = formatTime(m.startTime);
            const endTime = formatTime(m.endTime);
...
const MEETING_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbznasVEjsIC2BQus53B_l-R007-MAuNW2qgpf_RvRzn4b2fiM_9FRHkHoKqguAs7xHH/exec";
...
async function fetchWeeklyMissions() {
    ...
    try {
        ...
        let html = `<h3 style="color:var(--accent); margin-bottom:15px;">🚀 This Week's Meetings (Sun - Sat)</h3>
...
data.meetings.sort((a,b)=>{
    const aDate = new Date(`${a.date} ${a.startTime || "00:00"}`);
    const bDate = new Date(`${b.date} ${b.startTime || "00:00"}`);
    return aDate - bDate;
});
...
const startTime = cleanTime(m.startTime);
const endTime = cleanTime(m.endTime);
...
<div style="
    ...
    bottom: 0;
    width: 5px;
    background: ${isMyTeam ? 'var(--accent)' : 'white'};
    /* This ensures the bar curves only on the left side to match the card */
...
Lead : <b style="color:white">${m.by || "Unknown"}</b>
